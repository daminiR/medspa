...
// Pre-build search index for performance
...
export default function StaffListOptimized({ onSelectStaff, onAddStaff }: StaffListProps) {
  ...
  const itemsPerPage = 24; // Increased for better performance
  ...
  useEffect(() => {
    const loadStaff = async () => {
      ...
      // Simulate loading 500+ staff members for realistic testing
      const staffData = generateStaffDataset(500);
      const listItems = staffData.map(s => generateStaffListItem(s));
      
      // Build search index for O(1) lookups
      const index: SearchIndex[] = listItems.map(item => ({
        id: item.id,
        searchText: `${item.firstName} ${item.lastName} ${item.email} ${item.employeeId} ${item.role} ${item.specializations.join(' ')}`.toLowerCase(),
        item
      }));
  ...
  const filteredStaff = useMemo(() => {
    ...
    if (filter.search) {
      ...
      result = searchIndex
        .filter(entry => 
          searchWords.every(word => entry.searchText.includes(word))
        )
    ...
    // Apply other filters (these are typically faster)
    ...
    if (filter.specializations && filter.specializations.length > 0) {
      const specSet = new Set(filter.specializations);
      result = result.filter(s => 
        s.specializations.some(spec => specSet.has(spec))
      );
    }

    return result;
  }, [staff, searchIndex, filter, searching]);

  // Virtual scrolling for list view (only render visible items)
  ...
  useEffect(() => {
    if (currentPage < totalPages) {
      ...
      // Pre-calculate next page (browser will cache)
      filteredStaff.slice(nextPageStart, nextPageEnd);
    }
  }, [currentPage, totalPages, filteredStaff, itemsPerPage]);
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm">
              ...
              >
                ...
                <div className="space-y-1 text-sm">
                  ...
                  <div className="flex items-center gap-1 text-gray-500">
                    ...
                    <span className="text-xs truncate">{member.primaryLocation}</span>
                  </div>
      ...
      <div className="px-4 py-3 bg-gray-50 border-t text-xs text-gray-500">
        <div className="flex items-center justify-between">
          <span>Performance: {filteredStaff.length} results in {searching ? '...' : '<50ms'}</span>
