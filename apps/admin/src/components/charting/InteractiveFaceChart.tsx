...
interface QuickBubbleState {
  isOpen: boolean
  zoneId: string | null
  zone: TreatmentZone | null
  position: { x: number; y: number }
}
...
// Floating toolbar auto-hide timeout (3 seconds)
...
export function InteractiveFaceChart({
  ...
  const [quickBubble, setQuickBubble] = useState<QuickBubbleState>({ isOpen: false, zoneId: null, zone: null, position: { x: 0, y: 0 } })
  ...
  // Bubble input state (simple tap-to-bubble workflow)
  const [bubbleVisible, setBubbleVisible] = useState(false)
  const [bubblePosition, setBubblePosition] = useState<{ x: number; y: number } | null>(null)
  const [bubbleInputValue, setBubbleInputValue] = useState<string>('')
  ...
  // Scribble/Handwriting support detection
  const [scribbleSupported, setScribbleSupported] = useState(false)

  // Detect Scribble support on mount
  useEffect(() => {
    setScribbleSupported(detectScribbleSupport())
  }, [])
  ...
  // FOCUS/ZEN MODE - Distraction-free charting mode
  const [focusMode, setFocusMode] = useState(false)
  const [showFocusToolbar, setShowFocusToolbar] = useState(false)
  const focusToolbarTimeoutRef = useRef<NodeJS.Timeout | null>(null)
  ...
  const saveToLocalStorage = useCallback(() => {
    ...
    saveTimeoutRef.current = setTimeout(() => {
      try {
        const data = {
          injectionPoints: Array.from(injectionPoints.entries()).map(([, point]) => ({
            ...point,
  ...
  const handleZoneClick = useCallback((zone: TreatmentZone, event: React.MouseEvent) => {
    ...
    if (existingPoint) {
      ...
      toast.success(`Added ${zone.name}`, { duration: 1500, icon: '✓' })
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [injectionPoints, onInjectionPointsChange, productType, patientLastTreatment, selectedProductId, activeProducts, readOnly, selectionMode])
  ...
  const copyLastTreatment = useCallback(() => {
    ...
    onInjectionPointsChange(newPoints)
    toast.success('Copied previous treatment', { icon: '📋' })
  ...
  const loadPreviousPhoto = useCallback(() => {
    ...
    if (ctx) {
      ...
      toast.success('Loaded previous treatment photo (placeholder)', { icon: '👻' })
    }
  }, [])
  ...
  const handleMultiSelectZone = useCallback((zoneId: string, event?: React.MouseEvent) => {
    ...
    setMultiSelectedZones(prev => {
      ...
      // Show bubble near the last tapped zone
      if (newSet.size > 0 && event) {
        const rect = chartRef.current?.getBoundingClientRect()
        if (rect) {
          setBubblePosition({
  ...
  const applyBatchUnits = useCallback(() => {
    ...
    setShowBatchPanel(false)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [multiSelectedZones, multiSelectedFreehand, injectionPoints, freehandPoints, batchUnits, productType, activeDepths, activeTechniques, activeGauges, selectedProductId, activeProducts, getZoneById, onInjectionPointsChange, setFreehandPoints])

  const applyBubbleValue = useCallback((value: number) => {
    if (multiSelectedZones.size === 0 && multiSelectedFreehand.size === 0) return
    const newPoints = new Map(injectionPoints)
    const defaultDepth = activeDepths[0]?.id || ''
    const defaultTechnique = activeTechniques[0]?.id || ''
    const defaultGauge = activeGauges[0]?.id || ''
    multiSelectedZones.forEach(zoneId => {
      ...
      const existingPoint = newPoints.get(zoneId)
      if (existingPoint) {
        newPoints.set(zoneId, { ...existingPoint, units: productType === 'neurotoxin' ? value : undefined, volume: productType === 'filler' ? value : undefined })
      } else {
        newPoints.set(zoneId, { id: `ip-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`, zoneId, units: productType === 'neurotoxin' ? value : undefined, volume: productType === 'filler' ? value : undefined, depthId: zone?.defaultDepth || defaultDepth, techniqueId: zone?.defaultTechnique || defaultTechnique, needleGaugeId: zone?.defaultNeedleGauge || defaultGauge, productId: selectedProductId || activeProducts[0]?.id, timestamp: new Date() })
      }
    })
    pushToHistory(newPoints)
    onInjectionPointsChange(newPoints)
    if (multiSelectedFreehand.size > 0) {
      ...
      multiSelectedFreehand.forEach(pointId => {
        const point = newFreehand.get(pointId)
        if (point) newFreehand.set(pointId, { ...point, units: productType === 'neurotoxin' ? value : undefined, volume: productType === 'filler' ? value : undefined })
      })
      setFreehandPoints(newFreehand)
    }
    const totalSelected = multiSelectedZones.size + multiSelectedFreehand.size
    triggerHaptic('medium')
    speak(`${value} units applied`)
    toast.success(`Applied ${value} ${productType === 'neurotoxin' ? 'units' : 'ml'} to ${totalSelected} points`, { duration: 2000, icon: '✓' })
    setMultiSelectedZones(new Set())
    setMultiSelectedFreehand(new Set())
    setBubbleVisible(false)
    setBubbleInputValue('')
  }, [multiSelectedZones, multiSelectedFreehand, injectionPoints, freehandPoints, productType, activeDepths, activeTechniques, activeGauges, selectedProductId, activeProducts, getZoneById, onInjectionPointsChange, setFreehandPoints])
  ...
  const startVoiceInput = useCallback(() => {
    ...
    if (!recognition) {
      recognition = initVoiceInput() || null
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [voiceInput.isListening, initVoiceInput])
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [totals, audioFeedback, productType])
  ...
  // Show toolbar on interaction, auto-hide after delay on mobile/tablet
  const showToolbar = useCallback(() => {
    ...
    // Auto-hide only on tablet/mobile (< 1024px)
  ...
  // Show toolbar on any interaction with the chart
  useEffect(() => {
    ...
    if (chartElement) {
      chartElement.addEventListener('touchstart', handleInteraction)
      chartElement.addEventListener('mousedown', handleInteraction)
      chartElement.addEventListener('mousemove', handleInteraction)

      return () => {
        chartElement.removeEventListener('touchstart', handleInteraction)
        chartElement.removeEventListener('mousedown', handleInteraction)
        chartElement.removeEventListener('mousemove', handleInteraction)
      }
    }
  }, [showToolbar])
  ...
  const toggleFocusMode = useCallback(() => {
    setFocusMode(prev => {
      const newState = !prev
      if (newState) {
        toast.success('Focus mode enabled - Press F or Esc to exit', {
          icon: '🎯',
          duration: 2000
        })
        // Show toolbar briefly on entering focus mode
        setShowFocusToolbar(true)
        if (focusToolbarTimeoutRef.current) {
          clearTimeout(focusToolbarTimeoutRef.current)
        }
        focusToolbarTimeoutRef.current = setTimeout(() => {
  ...
  // Show focus toolbar on mouse movement in focus mode
  const handleFocusModeMouseMove = useCallback(() => {
    if (!focusMode) return
  ...
  // Cleanup focus toolbar timeout on unmount
  useEffect(() => {
    return () => {
      if (focusToolbarTimeoutRef.current) {
        clearTimeout(focusToolbarTimeoutRef.current)
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [injectionPoints, freehandPoints, readOnly])
          ...
          toast((t) => (
            <div className="flex flex-col gap-2">
              ...
              <div className="flex gap-2 mt-1">
                <button
                  onClick={() => {
                    const restoredInjectionPoints = new Map<string, InjectionPoint>(
                    ...
                    const restoredFreehandPoints = new Map<string, FreehandPoint>(
  ...
  const handleFreehandClick = useCallback((event: React.MouseEvent<HTMLDivElement>) => {
    ...
    toast.success('Added custom point', { duration: 1500, icon: '📍' })
  }, [readOnly, drawingMode, freehandPoints, setFreehandPoints, productType, selectedProductId, selectedProductColor, activeProducts, activeDepths, activeTechniques, activeGauges])
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      switch (e.key.toLowerCase()) {
        ...
        case 'enter':
          ...
          break
        case 'escape':
          // Exit focus mode, clear multi-selection, or exit speed mode
          if (focusMode) {
            e.preventDefault()
            toggleFocusMode()
            return
          ...
          break
        case 'f':
          // Toggle focus/zen mode (unless typing in an input)
          if (!voiceInput.isListening && !(e.target instanceof HTMLInputElement) && !(e.target instanceof HTMLTextAreaElement)) {
            e.preventDefault()
            toggleFocusMode()
            triggerHaptic('medium')
            return
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedZone, selectedFreehandPoint, injectionPoints, freehandPoints, productType, onSave, readOnly, voiceInput.isListening, selectionMode, multiSelectedZones, multiSelectedFreehand, activeZones, focusMode, toggleFocusMode])
  ...
  return (
    <div
      className={`h-full flex flex-col transition-all duration-300 ${focusMode ? 'bg-gray-900' : ''}`}
      ref={containerRef}
      onMouseMove={handleFocusModeMouseMove}
    >
      {/* RESPONSIVE LAYOUT - Face-Chart-First Philosophy */}
      {/*
        Breakpoints:
        - < 768px (Phone): Full-width chart, floating bottom toolbar
        - 768-1023px (Tablet Portrait): Centered chart, auto-hide toolbar
        - 1024-1279px (Tablet Landscape): 3-column with collapsible sidebars
        - >= 1280px (Desktop): Full 3-column, chart dominates 65-70%
      */}

      {/* Main Layout - Face Chart Dominates (65-70% width on desktop) */}
      <div className="flex-1 flex flex-col lg:flex-row gap-0 overflow-hidden">
        {/* Face Chart Container - PRIMARY ELEMENT (70% on desktop, 100% on mobile/focus mode) */}
        <div className={`flex-1 ${!focusMode ? 'lg:flex-[0_0_70%]' : ''} bg-white border-b lg:border-b-0 lg:border-r border-gray-200 shadow-sm overflow-hidden relative flex flex-col`}>
        {/* Minimal Toolbar - TABLET OPTIMIZED */}
        {/* Phone: Hidden, Tablet Portrait: Auto-hide after 3s, Tablet Landscape+: Always visible */}
        {/* Focus Mode: Only visible when showFocusToolbar is true */}
        <div className={`
          hidden sm:flex items-center justify-between px-4 md:px-6 py-3 md:py-4 border-b border-gray-100
          bg-white
          transition-all duration-300 ease-in-out
          ${focusMode ? 'fixed top-0 left-0 right-0 z-50 shadow-lg' : ''}
          ${focusMode && !showFocusToolbar ? 'opacity-0 pointer-events-none -translate-y-full' : ''}
          ${!focusMode && !isToolbarVisible ? 'md:max-lg:opacity-0 md:max-lg:pointer-events-none md:max-lg:-translate-y-2' : ''}
        `}>
          <div className="flex items-center gap-3 md:gap-4">
            <h3 className="font-medium text-base md:text-lg text-gray-900">Face Injection Map</h3>
            ...
            <div className="flex items-center bg-gray-100 rounded-lg p-0.5">
              <button
                ...
                className={`px-2 md:px-3 py-2 md:py-1.5 text-xs font-medium rounded-md flex items-center gap-1 md:gap-1.5 transition-all touch-target-feedback ${
                  drawingMode === 'zones'
              ...
              <button
                ...
                title="Tap anywhere to place injection points"
              >
          ...
          <div className="flex items-center gap-1 md:gap-2 overflow-x-auto">
            ...
            <button
              ...
              className="px-2 md:px-3 py-2 md:py-1.5 text-xs md:text-sm bg-purple-50 text-purple-700 rounded-lg hover:bg-purple-100 flex items-center gap-1 md:gap-1.5 transition-colors whitespace-nowrap touch-target-feedback"
            >
            ...
            <button
              ...
              className={`px-2 md:px-3 py-2 md:py-1.5 text-xs md:text-sm rounded-lg flex items-center gap-1 md:gap-1.5 transition-all shadow-sm whitespace-nowrap touch-target-feedback ${
                selectionMode === 'multi'
            ...
            {patientLastTreatment && patientLastTreatment.size > 0 && (
              <button
                ...
                className="px-2 md:px-3 py-2 md:py-1.5 text-xs md:text-sm text-gray-600 hover:text-gray-800 hover:bg-gray-100 rounded-lg flex items-center gap-1 md:gap-1.5 transition-colors whitespace-nowrap touch-target-feedback"
                title="Copy previous treatment (Ctrl+D)"
              >
                <Copy className="w-3.5 h-3.5 md:w-4 md:h-4" />
                <span className="hidden lg:inline">Copy Last</span>
              </button>
            )}
            ...
            >
              ...
              <span className="hidden lg:inline">Ghost Overlay</span>
            </button>
            ...
            <button
              ...
              title="Focus Mode - Hide distractions (Press F)"
            >
              ...
              {activeTemplates.map(template => (
                ...
                >
                  ...
                  <p className="text-xs text-gray-500">
                    {template.zones.length} zones • {template.estimatedTime}min
                  </p>
                </button>
              ))}
            </div>
                ...
                <div className="flex gap-2 mt-3">
                  ...
                  <button
                    onClick={() => {
                      ...
                      toast.success('Ghost overlay removed', { icon: '✓' })
                    }}
          ...
          <div className="px-4 py-3 bg-gray-50 border-b border-gray-100 text-xs">
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-x-4 gap-y-1">
                ...
                <span><kbd className="px-1.5 py-0.5 bg-gray-200 rounded">F</kbd> Focus Mode</span>
        ...
        <div className="relative p-2 md:p-4 min-h-[50vh] md:min-h-[70vh] lg:min-h-0" ref={chartRef}>
          ...
          {/* Bubble Input (Tap-to-Bubble Workflow) */}
          {bubbleVisible && bubblePosition && (
            <div
              className="absolute z-[60] animate-fade-in"
              style={{
                left: `${bubblePosition.x + 20}px`,
                top: `${bubblePosition.y - 10}px`,
                pointerEvents: 'auto'
              }}
            >
              <div className="bg-white rounded-xl shadow-2xl border-2 border-purple-400 p-3 min-w-[200px]">
                <div className="flex items-center gap-2 mb-2">
                  <Target className="w-4 h-4 text-purple-600" />
                  <span className="text-xs font-semibold text-purple-700">
                    {multiSelectedZones.size + multiSelectedFreehand.size} selected
                  </span>
                  <button
                    onClick={() => {
                      setBubbleVisible(false)
                ...
                <div className="grid grid-cols-4 gap-1 mb-2">
                  {productType === 'neurotoxin' ? (
                    <>
                      <button onClick={() => applyBubbleValue(2)} className="px-2 py-1.5 text-xs font-semibold bg-purple-100 hover:bg-purple-200 text-purple-700 rounded transition-colors">2u</button>
                      <button onClick={() => applyBubbleValue(4)} className="px-2 py-1.5 text-xs font-semibold bg-purple-100 hover:bg-purple-200 text-purple-700 rounded transition-colors">4u</button>
                      <button onClick={() => applyBubbleValue(5)} className="px-2 py-1.5 text-xs font-semibold bg-purple-100 hover:bg-purple-200 text-purple-700 rounded transition-colors">5u</button>
                      <button onClick={() => applyBubbleValue(10)} className="px-2 py-1.5 text-xs font-semibold bg-purple-100 hover:bg-purple-200 text-purple-700 rounded transition-colors">10u</button>
                    </>
                  ) : (
                    <>
                      <button onClick={() => applyBubbleValue(0.1)} className="px-2 py-1.5 text-xs font-semibold bg-pink-100 hover:bg-pink-200 text-pink-700 rounded transition-colors">0.1</button>
                      <button onClick={() => applyBubbleValue(0.2)} className="px-2 py-1.5 text-xs font-semibold bg-pink-100 hover:bg-pink-200 text-pink-700 rounded transition-colors">0.2</button>
                      <button onClick={() => applyBubbleValue(0.5)} className="px-2 py-1.5 text-xs font-semibold bg-pink-100 hover:bg-pink-200 text-pink-700 rounded transition-colors">0.5</button>
                      <button onClick={() => applyBubbleValue(1.0)} className="px-2 py-1.5 text-xs font-semibold bg-pink-100 hover:bg-pink-200 text-pink-700 rounded transition-colors">1.0</button>
                    </>
                  )}
                </div>
                ...
                <div className="flex gap-1">
                  <input
                    ...
                    onChange={(e) => setBubbleInputValue(e.target.value)}
                    onKeyDown={(e) => {
                      if (e.key === 'Enter' && bubbleInputValue) {
                        applyBubbleValue(parseFloat(bubbleInputValue))
                      }
                    }}
                    step={productType === 'neurotoxin' ? 1 : 0.1}
                    min="0"
                    placeholder="Custom"
                    className="flex-1 px-2 py-1.5 text-sm border border-gray-300 rounded focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                  />
                  <button
                    onClick={() => {
                      if (bubbleInputValue) {
                    ...
                    className="px-3 py-1.5 bg-purple-600 hover:bg-purple-700 text-white rounded font-semibold text-sm transition-colors"
                  >
          ...
          {/* Face Image Background - TABLET OPTIMIZED */}
          {/* Tablet Portrait (768-1023px): Centered chart, max 700px */}
          {/* Tablet Landscape (1024px+): Larger centered chart, max 900px */}
          <div
            ref={imageContainerRef}
            className={`relative w-full mx-auto transition-all duration-300 chart-container
          ...
          >
            ...
            <div className="absolute inset-0 chart-area" style={{ pointerEvents: drawingMode === 'zones' ? 'auto' : 'none' }}>
              {activeZones.map(zone => {
                ...
                if (!hasInjection && !isMultiSelected) return null

                return (
                  <div
                    key={zone.id}
                    className={`absolute transform -translate-x-1/2 -translate-y-1/2 group touch-zone ${
                    ...
                    onClick={(e) => {
                      if (drawingMode === 'zones') {
                        e.stopPropagation()
                        if (selectionMode === 'multi') {
                          handleMultiSelectZone(zone.id, e)
                        } else {
                  ...
                  >
                    {/* Touch-friendly clickable area - Tablet optimized: 44x44px minimum */}
                    {/* Mobile: 56px (w-14), Tablet: 48px (w-12), Desktop: 44px (w-11) */}
                    <div className="absolute w-14 h-14 md:w-12 md:h-12 lg:w-11 lg:h-11 -left-7 md:-left-6 lg:-left-5.5 -top-7 md:-top-6 lg:-top-5.5 touch-manipulation" />

                    {/* Visual Point - Uses product-specific color */}
                    {(() => {
                      // Get product-specific color for this injection point
                      const pointColor = point?.productId
                        ? getProductColor(point.productId)
                      ...
                      return (
                        <div
                          className={`
                            ...
                            ${isMultiSelected
                              ? 'ring-2 ring-offset-2 ring-offset-white scale-125 shadow-lg'
                              : isSelected
                                ? 'ring-4 scale-150'
                                : 'shadow-lg scale-125'
                            }
                    ...
                    {hasInjection && selectionMode !== 'multi' && (
                      <div
                        className="absolute -top-8 left-1/2 transform -translate-x-1/2 px-2.5 py-1 rounded-full text-xs font-bold text-white shadow-lg whitespace-nowrap z-10"
                        style={{
                          backgroundColor: point?.productId ? getProductColor(point.productId) : (productType === 'neurotoxin' ? '#8B5CF6' : '#EC4899')
                        }}
                      >
                ...
                return (
                  ...
                  >
                    ...
                    {(() => {
                      ...
                      return (
                        <div
                          className="absolute -top-9 left-1/2 transform -translate-x-1/2 px-2 py-1 rounded-lg text-xs font-medium text-white shadow-lg whitespace-nowrap z-10"
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-blue-500 font-medium">Placed</span>
                    </div>
        ...
        {/* Floating Toolbar Indicator - TABLET PORTRAIT ONLY */}
        {/* Shows when toolbar is hidden, tapping reveals it */}
        {!isToolbarVisible && (
          <div className="hidden md:max-lg:block fixed bottom-4 right-4 z-50">
            <button
              onClick={showToolbar}
              className={`
    ...
    {/* Right Sidebar - Tools & Summary (30% on desktop, complements 70% chart) */}
      {/* Phone/Tablet Portrait: Hidden, Tablet Landscape+: Always visible */}
      {/* Focus Mode: Hidden */}
      {!focusMode && (
      <div className="hidden lg:block lg:flex-[0_0_30%] bg-gray-50/30 border-l border-gray-200 overflow-y-auto p-3 md:p-4 space-y-3">
        ...
        {selectedZone && injectionPoints.has(selectedZone) && (
          <QuickEditPanel
            ...
            scribbleSupported={scribbleSupported}
          />
        )}
                ...
                <div className="relative">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      const parsed = parseHandwrittenInput(rawValue)
                      if (parsed !== null) {
                        setBatchUnits(parsed)
                      } else {
                        setBatchUnits(parseFloat(rawValue) || 0)
                      }
                    }}
                    step={productType === 'neurotoxin' ? 1 : 0.1}
                    min="0"
                  ...
                  />
                  {scribbleSupported && (
                    <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1 text-xs text-purple-500 pointer-events-none">
                    ...
                    {Array.from(multiSelectedFreehand).map(pointId => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="text-blue-600 text-xs">Placed</span>
                        </div>
                      )
                    })}
                  </div>
      ...
      {/* Floating Toolbar - Auto-hides on mobile/tablet */}
      ...
      <div
        className={`
          ...
          transition-all duration-300 ease-in-out
          ${isToolbarVisible ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0 pointer-events-none'}
      ...
      >
        <div className="
          ...
          min-h-[56px]
        ">
          {/* Product Selector - Opens modal/sheet */}
          <button
            onClick={() => setShowTemplates(!showTemplates)}
            className="flex flex-col items-center justify-center min-w-[48px] min-h-[44px] px-2 py-1 rounded-lg hover:bg-gray-100 active:scale-95 transition-all touch-target-feedback"
          ...
          >
            ...
            <span className="text-[10px] text-gray-600 mt-0.5">Prod</span>
          </button>

          {/* Drawing Mode Toggle */}
          <button
            onClick={() => setDrawingMode(drawingMode === 'zones' ? 'freehand' : 'zones')}
            className="flex flex-col items-center justify-center min-w-[48px] min-h-[44px] px-2 py-1 rounded-lg hover:bg-gray-100 active:scale-95 transition-all touch-target-feedback"
            title={drawingMode === 'zones' ? 'Switch to Place mode' : 'Switch to Zones'}
          >
            {drawingMode === 'zones' ? (
            ...
            <span className="text-[10px] text-gray-600 mt-0.5">Tool</span>
          </button>
          ...
          <button
            onClick={undo}
            disabled={historyIndex <= 0}
            className="flex flex-col items-center justify-center min-w-[48px] min-h-[44px] px-2 py-1 rounded-lg hover:bg-gray-100 active:scale-95 transition-all disabled:opacity-30 disabled:cursor-not-allowed touch-target-feedback"
            title="Undo (Ctrl+Z)"
          >
          ...
          <button
            ...
            title="Redo (Ctrl+Shift+Z)"
          >
            <RotateCcw className="w-5 h-5 text-gray-700 scale-x-[-1]" />
          ...
          {!readOnly && onSave && (
            <button
              onClick={onSave}
              className="flex flex-col items-center justify-center min-w-[48px] min-h-[44px] px-3 py-1 rounded-lg bg-gradient-to-r from-purple-500 to-pink-500 text-white hover:from-purple-600 hover:to-pink-600 active:scale-95 transition-all shadow-md touch-target-feedback"
              title="Save Chart"
            >
              <Save className="w-5 h-5" />
              <span className="text-[10px] mt-0.5 font-medium">Save</span>
            </button>
          )}

          {/* More Options - Opens settings/options */}
          <button
            onClick={() => setShowKeyboardHelp(!showKeyboardHelp)}
            className="flex flex-col items-center justify-center min-w-[48px] min-h-[44px] px-2 py-1 rounded-lg hover:bg-gray-100 active:scale-95 transition-all touch-target-feedback"
            title="More Options"
          >
            ...
            <span className="text-[10px] text-gray-600 mt-0.5">More</span>
          </button>
        </div>
      </div>
      )}
...
// =============================================================================
// QUICK BUBBLE PRESET SELECTOR COMPONENT
// =============================================================================
...
function QuickBubble({
  ...
  useEffect(() => {
    ...
    if (visible) {
      // Add slight delay to prevent immediate close from the zone click
      const timer = setTimeout(() => {
  ...
  return (
    <div
      ...
      style={{
        left: position.x,
        top: position.y,
        transform: 'translate(-50%, -120%)' // Position above the clicked zone
      }}
    >
      {/* Pointer arrow */}
      <div className="relative">
        <div
          className={`absolute left-1/2 -translate-x-1/2 -bottom-2 w-0 h-0 border-l-8 border-r-8 border-t-8 border-l-transparent border-r-transparent ${
            productType === 'neurotoxin' ? 'border-t-purple-100' : 'border-t-pink-100'
          }`}
        />
      </div>

      {/* Main bubble */}
      <div className={`rounded-xl shadow-2xl overflow-hidden min-w-[280px] ${
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-600">
                Select {productType === 'neurotoxin' ? 'units' : 'volume (ml)'}
              </p>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="grid grid-cols-4 gap-2">
            {presets.map((preset) => (
              <button
                ...
                className={`py-2.5 px-2 rounded-lg text-sm font-semibold transition-all transform hover:scale-105 active:scale-95 ${
                  productType === 'neurotoxin'
                    ? 'bg-gradient-to-br from-purple-500 to-purple-600 hover:from-purple-600 hover:to-purple-700 text-white shadow-md'
                    : 'bg-gradient-to-br from-pink-500 to-pink-600 hover:from-pink-600 hover:to-pink-700 text-white shadow-md'
                }`}
                style={{ minHeight: '44px' }} // Touch-friendly height
              >
                {productType === 'neurotoxin' ? `${preset}u` : `${preset.toFixed(1)}ml`}
              </button>
            ))}
          </div>
          ...
          <div className="pt-2 border-t border-gray-200">
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder={productType === 'neurotoxin' ? '20' : '1.0'}
                step={productType === 'neurotoxin' ? '1' : '0.1'}
                min="0"
            ...
            <p className="text-xs text-gray-500 mt-1.5">
              Supports Apple Pencil Scribble
            </p>
          </div>
        </div>
      </div>
...
// =============================================================================
// FREEHAND POINT QUICK EDIT COMPONENT (duplicate removed - using QuickBubble)
// =============================================================================
          ...
          <div className="space-y-3 pt-2 border-t border-gray-100">
            ...
            <div>
              <label className="text-xs font-medium text-gray-500 mb-1 block flex items-center gap-1">
                ...
                Notes
                {scribbleSupported && (
                  <span className="text-purple-500 flex items-center gap-1 ml-auto">
                    <PenTool className="w-3 h-3" />
      ...
      <div className={`px-4 py-3 flex items-center justify-between ${
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative">
            ...
            />
            <p className="text-xs text-blue-500 font-medium">Freehand Point{scribbleSupported && ' · Scribble'}</p>
          </div>
        </div>
        <button
          onClick={onClose}
