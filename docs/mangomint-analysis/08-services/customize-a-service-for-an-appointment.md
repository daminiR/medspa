# Customize A Service For An Appointment

**URL:** https://www.mangomint.com/learn/customize-a-service-for-an-appointment/

## Category
08 Services

## Subcategory
N/A

## Article Content

HOME
SERVICES
SERVICE CUSTOMIZATIONS
Customize a Service for an Appointment

After adding a service customization to a service, the service can be customized at the time of booking from both the calendar and online booking. Service customizations can also be updated during checkout.

Link copied
Customizing a service from the calendar

To customize a service from the calendar, select the service and update the customizations as shown below. During checkout, you can update the customizations if needed.

Note: 

An asterisk (*) indicates that a selection for that customization is required.

Link copied
Customizing a service in online booking

After a client selects the service in online booking, they can customize the service before booking the appointment.

Can't find what you're looking for?

Start a chat with us to talk to a real person and get your questions answered, or browse our on-demand videos.

© Mangomint, Inc. All rights reserved.

## Images and Screenshots

This article contains the following images:

1. **Untitled image**
   - Source: data:image/png;base64,

2. **Start a chat with us**
   - Source: https://www.mangomint.com/static/19c674dff2afe2804ca05e52a5b8744c/e4a6b/chat-icon.png

3. **Untitled image**
   - Source: https://bat.bing.com/action/0?ti=97065788&Ver=2&mid=0fb652d0-bfe0-44c1-9793-c947a4499a21&bo=1&sid=6470ad60aad411f0a08c3337922d37be&vid=6470f430aad411f0ad3dbbd763893d96&vids=0&msclkid=N&pi=0&lg=en-US&sw=1920&sh=1080&sc=24&tl=Customize%20a%20Service%20for%20an%20Appointment%20%7C%20Mangomint%20Salon%20and%20Spa%20Software&p=https%3A%2F%2Fwww.mangomint.com%2Flearn%2Fcustomize-a-service-for-an-appointment%2F&r=&lt=319&evt=pageLoad&sv=2&cdb=AQAQ&rn=133270


## Analysis Notes
<!-- Add your analysis notes here -->

## Key Features Identified
<!-- List key features mentioned in this article -->

## Competitor Insights
<!-- Note what this reveals about Mango Mint's capabilities -->
