# How To Merge Duplicate Clients

**URL:** https://www.mangomint.com/learn/how-to-merge-duplicate-clients/

## Category
04 Clients

## Subcategory
N/A

## Article Content

HOME
CLIENTS
CLIENT LIST
Merging Duplicate Clients

To combine duplicate client profiles, open the Clients app and select the client you want to keep.

Select Merge Client from the "..." menu.

Specify the client to merge from and select Next.

Select Confirm to merge the clients.

Important: 

The client to merge from will be archived. This action cannot be undone.

The following information will be merged:

Appointments

Cards on file

Client notes (will be combined with any existing client notes)

Gift cards

Sales

Timeline notes

Any information that does not exist in the Client to merge into will be copied from the Client to merge from, but the system will not override any information.

Can't find what you're looking for?

Start a chat with us to talk to a real person and get your questions answered, or browse our on-demand videos.

© Mangomint, Inc. All rights reserved.

## Images and Screenshots

This article contains the following images:

1. **Untitled image**
   - Source: data:image/svg+xml;charset=utf-8,%3Csvg%20height='667'%20width='640'%20xmlns='http://www.w3.org/2000/svg'%20version='1.1'%3E%3C/svg%3E

2. **Untitled image**
   - Source: data:image/jpeg;base64,

3. **Untitled image**
   - Source: https://images.ctfassets.net/oco7uqwlyque/4yApz6LBONSzVfJWRACjla/a9d37ed39fb6021d5d74d9d692383dae/lc-merge-duplicate-clients-1.jpg?w=640&h=667&fl=progressive&q=90&fm=jpg&bg=transparent

4. **Untitled image**
   - Source: data:image/svg+xml;charset=utf-8,%3Csvg%20height='519'%20width='640'%20xmlns='http://www.w3.org/2000/svg'%20version='1.1'%3E%3C/svg%3E

5. **Untitled image**
   - Source: data:image/jpeg;base64,

6. **Untitled image**
   - Source: https://images.ctfassets.net/oco7uqwlyque/36YrXDFQs42RLHp1a22Gny/86c1738996403f8c4b052820c72cd680/lc-merge-duplicate-clients-2.jpg?w=640&h=519&fl=progressive&q=90&fm=jpg&bg=transparent

7. **Untitled image**
   - Source: data:image/png;base64,

8. **Start a chat with us**
   - Source: https://www.mangomint.com/static/19c674dff2afe2804ca05e52a5b8744c/e4a6b/chat-icon.png

9. **Untitled image**
   - Source: https://bat.bing.com/action/0?ti=97065788&Ver=2&mid=3fe2ac9b-eee1-4c3c-95c0-32820c2aa2fd&bo=1&sid=6470ad60aad411f0a08c3337922d37be&vid=6470f430aad411f0ad3dbbd763893d96&vids=0&msclkid=N&pi=0&lg=en-US&sw=1920&sh=1080&sc=24&tl=Merging%20Duplicate%20Clients%20%7C%20Mangomint%20Salon%20and%20Spa%20Software&p=https%3A%2F%2Fwww.mangomint.com%2Flearn%2Fhow-to-merge-duplicate-clients%2F&r=&lt=349&evt=pageLoad&sv=2&cdb=AQAQ&rn=445056


## Analysis Notes
<!-- Add your analysis notes here -->

## Key Features Identified
<!-- List key features mentioned in this article -->

## Competitor Insights
<!-- Note what this reveals about Mango Mint's capabilities -->
