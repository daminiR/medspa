# Hipaa Compliance

**URL:** https://www.mangomint.com/learn/hipaa-compliance/

## Category
01 Getting Started

## Subcategory
N/A

## Article Content

HOME
GETTING STARTED
ABOUT MANGOMINT
HIPAA Compliance

Mangomint is a HIPAA compliant software that supports the privacy and security needs of medical spas and related businesses.

With Mangomint, your business can handle sensitive Protected Health Information (PHI) in full accordance with the U.S. Health Insurance Portability and Accountability Act (HIPAA).

Link copied
HIPAA Compliance Checklist

As the business owner, here’s what you need to do to become HIPAA compliant with Mangomint:

Contact Mangomint via chat support and let us know your business requires HIPAA compliance.

Review and sign the required Business Associate Agreement (BAA) provided by Mangomint. Once you’ve signed the document, the Mangomint team will confirm with you once your account is fully HIPAA compliant.

Mangomint will sign the BAA and send you a copy of the document for your records.

Important: 

Protected Health Information (PHI) should not be sent via text message in Mangomint. (PLACEHOLDER SEE ERROR LOG) is not HIPAA compliant.

Can't find what you're looking for?

Start a chat with us to talk to a real person and get your questions answered, or browse our on-demand videos.

© Mangomint, Inc. All rights reserved.

## Images and Screenshots

This article contains the following images:

1. **Untitled image**
   - Source: data:image/png;base64,

2. **Start a chat with us**
   - Source: https://www.mangomint.com/static/19c674dff2afe2804ca05e52a5b8744c/e4a6b/chat-icon.png

3. **Untitled image**
   - Source: https://bat.bing.com/action/0?ti=97065788&Ver=2&mid=d973f12a-59ee-4e97-8bf9-1adace6ebbee&bo=1&sid=6470ad60aad411f0a08c3337922d37be&vid=6470f430aad411f0ad3dbbd763893d96&vids=0&msclkid=N&pi=0&lg=en-US&sw=1920&sh=1080&sc=24&tl=HIPAA%20Compliance%20%7C%20Mangomint%20Salon%20and%20Spa%20Software&p=https%3A%2F%2Fwww.mangomint.com%2Flearn%2Fhipaa-compliance%2F&r=&lt=405&evt=pageLoad&sv=2&cdb=AQAQ&rn=72970


## Analysis Notes
<!-- Add your analysis notes here -->

## Key Features Identified
<!-- List key features mentioned in this article -->

## Competitor Insights
<!-- Note what this reveals about Mango Mint's capabilities -->
