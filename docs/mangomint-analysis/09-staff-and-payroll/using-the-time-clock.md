# Using The Time Clock

**URL:** https://www.mangomint.com/learn/using-the-time-clock/

## Category
09 Staff And Payroll

## Subcategory
N/A

## Article Content

HOME
STAFF MEMBERS
TIME CLOCK
Using the Time Clock

After setting up the time clock, staff members can clock in and out and use task types.

If a staff member has the Can view Time Clock app permission enabled, they can clock in and out from any device where they are logged in. If a staff member does not have this permission enabled, they can still clock in using their PIN in another account where the Time Clock app is visible, such as a front desk device.

Link copied
Clocking in and out

To clock in, open the Time Clock app.

Enter your PIN that was assigned to you by your Admin and select Next. Select Clock In.

To clock out, repeat these same steps and select Clock Out.

Link copied
Using task types

After setting up task types, you can use them in the Time Clock app.

To use a task type, make sure you are currently clocked in. Once you are clocked in, open the Time Clock app. Enter your PIN and select Next.

To begin a task type, select it from the available task types. To complete the task type, enter your PIN and select Next. Select End.

Can't find what you're looking for?

Start a chat with us to talk to a real person and get your questions answered, or browse our on-demand videos.

© Mangomint, Inc. All rights reserved.

## Images and Screenshots

This article contains the following images:

1. **Untitled image**
   - Source: data:image/png;base64,

2. **Start a chat with us**
   - Source: https://www.mangomint.com/static/19c674dff2afe2804ca05e52a5b8744c/e4a6b/chat-icon.png

3. **Untitled image**
   - Source: https://bat.bing.com/action/0?ti=97065788&Ver=2&mid=3d284355-5154-4251-8bdd-7a2ee3b2f62b&bo=1&sid=6470ad60aad411f0a08c3337922d37be&vid=6470f430aad411f0ad3dbbd763893d96&vids=0&msclkid=N&pi=0&lg=en-US&sw=1920&sh=1080&sc=24&tl=Using%20the%20Time%20Clock%20%7C%20Mangomint%20Salon%20and%20Spa%20Software&p=https%3A%2F%2Fwww.mangomint.com%2Flearn%2Fusing-the-time-clock%2F&r=&lt=360&evt=pageLoad&sv=2&cdb=AQAQ&rn=589052


## Analysis Notes
<!-- Add your analysis notes here -->

## Key Features Identified
<!-- List key features mentioned in this article -->

## Competitor Insights
<!-- Note what this reveals about Mango Mint's capabilities -->
