...
*Source: [https://jane.app/guide/multiple-locations-or-multiple-accounts-how-to-set-up-jane-for-independent-locations-or-practitioners](https://jane.app/guide/multiple-locations-or-multiple-accounts-how-to-set-up-jane-for-independent-locations-or-practitioners)*

*Category: Account Basics*

*Scraped: 2025-08-06 23:33:08*
...
Feature OverviewMultiple Locations or Multiple Accounts: How to Set Up Jane for Independent Locations or Practitioners
Account Basics
Multiple Locations or Multiple Accounts: How to Set Up Jane for Independent Locations or Practitioners
7 min read
Practice Plan
...
Now that you’ve
signed up for Jane
, you might be wondering if you need multiple locations within a single Jane account or multiple accounts.  Here are some questions to consider:
Multiple counsellors or practitioners sharing a single practice space but are responsible for their own admin and data
One single owner with multiple physical locations that do not share practitioners, branding or client lists
One single owner with multiple physical locations that do share practitioners, branding and client lists (like a Chain)
One single owner with a mobile practice that works out of different locations (corporate, mobile or home-based, for example)
In the above four scenarios, the recommendation for the top two is to open multiple, unique accounts, while the bottom two use Jane’s
Locations
feature.  Typically, the top two would have separate bank accounts, financials, and admin staff. The bottom two would want aggregated reporting data as well as a shared client and staff list.
The most important point when there’s multiple physical locations or independent practitioners working together is that while a filtered patient list, appointments, clinical notes and past billing information can all be exported from Jane by practitioner or by location,
the data will remain in the primary account, and Billing Information cannot be transported over to a new account
. (Note - patient list can be filtered by practitioner but not by location).
Pain Points
A few pain points that should be assessed:
Patient Accessibility:
Are the same clients seen across multiple locations?  If so, the Locations feature allows for the same username and password to be used to book at both locations.
Chart Accessibility:
With a single account, you would see one unified medical chart for clients that are seen across multiple locations.  Multiple accounts would have a separate chart for the client at each location.
Change in Ownership:
If you sold your practice, how would you want all the data to be divided? If you plan on selling one location and keeping another, then multiple accounts would make sense for a simple transition of data.
Access of Data:
Jane offers many different
access levels
that allow you to limit the information available to everyone with administrative access to your Jane Account. Reports and Exports can be limited to individual information only, or can be set to allow for more shared access.
Admin Access and Reporting:
Do you have shared admin across all practitioners?  It can be hard for admin staff to manage multiple accounts if they are booking and receiving payment for your clients across multiple locations.  A single account makes it simpler for admin and also allows for aggregated data on reports across all practitioners.
...
Logo and Branding:
There is only one logo and branding available per account.  So a single shared account will need to have shared branding.  Some people are managing this using a split logo file where they’ve created a single file that has multiple logos side by side.
Email communication:
When Jane emails out the notifications and reminders to clients, the clients can respond back to those emails and Jane will redirect them to the clinic’s email address. However, there can only be one reply-to email address per Jane account. Every location will use this reply-to address.
Billing for the License Fee:
We can only charge a single card per Account for the Jane license fee - so if you are sharing an account, you would need to manage the splitting of the license fee outside of Jane.
Pricing
One Account with Multiple Locations
Jane’s pricing is license-based, and there is no cost to add a location to your existing Jane account. However, any
additional or new practitioners
working at the new location would be billed at an additional license fee, depending on your subscription plan.
Here are some handy guides that walk through how to add a new location and more details on how licenses work:
Adding a Location
FAQ: Subscription Licenses and Jane’s Usage Report
Multiple Jane Accounts
If you have multiple Jane accounts, they are quoted and priced completely separately. If five practitioners have five accounts, then each account is priced individually.
...
The second point for us is that running additional accounts comes with extra costs to Jane for importing, training, and hosting.
...
Security
How to reset your email password and enable Two-Factor Authentication (2FA)Multiple Locations or Multiple Accounts: How to Set Up Jane for Independent Locations or Practitioners
Account Basics
Multiple Locations or Multiple Accounts: How to Set Up Jane for Independent Locations or Practitioners
7 min read
Practice Plan
Thrive PlanNow that you’ve
signed up for Jane
, you might be wondering if you need multiple locations within a single Jane account or multiple accounts.  Here are some questions to consider:
Multiple counsellors or practitioners sharing a single practice space but are responsible for their own admin and data
One single owner with multiple physical locations that do not share practitioners, branding or client lists
...
Pricing Page
for an overview of the different plan types and their list of available features.
If you are really unsure about whether it would be better for you to work with one account or multiple accounts for your practice, please don’t hesitate to contact us and describe your unique practice makeup.  We do like to stress that in most cases, there are pros and cons for BOTH setups, so ultimately, you will be the best one to decide which direction you’d like to go.Subscribe to our monthly newsletter.Jane Software Inc.
