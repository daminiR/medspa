...
*Source: [https://jane.app/guide/package-membership-reports](https://jane.app/guide/package-membership-reports)*

*Category: Packages & Memberships*

*Scraped: 2025-08-07 04:46:21*
...
Feature OverviewPackage & Membership Reports
Packages & Memberships
Package & Membership Reports
5 min read
Thrive Plan
Now that you’ve gotten all of your
packages
and
memberships
set up, you may be interested in learning more about which passes have been selling well, which are on the brink of expiry, which types of services are redeemed most often or even just a list of which patients have passes.
There are two reports in Jane that will help us explore this information, the
Package & Memberships Sales
and
Usage
Reports.
📣 There have been some changes since this video was recorded. These reports are now called
Package & Membership Sales and Usage Reports
within the left sidebar navigation.
Package & Membership Sales Report
Looking for a list of patients with memberships or a package? The
Package & Membership Sales
report provides an overview of all packages and memberships that have been sold within a particular date range, including the status of each package and membership.
Location:
The location where the package or membership invoice was billed at.
Patient Name:
The patient that the package or membership has been invoiced to.
Purchase Date:
The Purchase Date of when the package or membershiuwas invoiced to the patient. If you’re interested in learning more about the different types of dates in Jane, check out our handy guide on
Working with the Purchase, Invoice, Received At, and Applied Dates
.
Remaining Treatments:
The total number of remaining sessions that this pass has available for use.
Redeemed Treatments:
The number of sessions that have been redeemed for this particular pass.
Status:
Is the pass still active? In other words, is today’s date after or before the expiration date for this particular package?
Expiration Date:
When the package is due to expire, or membership is due to terminate. This is determined by the preferences set out when setting up a particular pass, but can modified on an individual basis.
Last Treatment:
The treatment type that was last redeemed by using this pass.
Last Used:
The date when the pass was last used.
Outstanding Balance:
Indicates whether there is an unpaid amount associated with the package or membership.
Clinics will also be able to narrow down this report using the following filters:
All Passes:
Display passes regardless of the number of sessions that have been redeemed.
With Treatments Remaining:
Display passes that have sessions remaining and have not been redeemed in full. Another way of thinking about this is that Jane is bringing up any passes where the
Remaining Sessions
value is  greater than zero.
Completed:
Only display packages where all of the sessions have been redeemed, or in other words, where the
Remaining Sessions
value is zero.
All Statuses:
Display packages regardless of whether it is expired or not.
Active
: Only display packages that have not yet expired.
Expired:
Only display packages that have already expired.
Package & Membership Usage Report
The
Package & Membership Usage
report was designed to provide clinics with a more in-depth look at each of the individual sessions that have been covered using an existing pass. Let’s review the information that this report has to offer:
Purchase Date:
The Purchase Date (or date of service) of when the treatment or class was invoiced to the patient. If you’re interested in learning more about the different types of dates in Jane, check out our handy guide on
Working with the Purchase, Invoice, Received At, and Applied Dates
.
Pass Type:
Is this a package or membership?
Pass Name:
The name of the package that was redeemed to cover this session.
Patient:
The name of the patient for whom the appointment was for.
Redeemed For:
The particular treatment type that this appointment was booked for.
Staff Member:
The staff member who offered this treatment.
Invoice Number:
The invoice associated with the appointment.
Original Value:
The original value of the session prior to any pass being redeemed.
Discount:
The savings offered to a patient as a result of the package being redeemed. Formally calculated as:
Additional Charge
(from
Settings
>
...
Clinics will also be able to narrow down this report using the staff member, pass type, and date range filters.
...
Security
How to reset your email password and enable Two-Factor Authentication (2FA)Package & Membership Reports
Packages & Memberships
Package & Membership Reports
5 min read
Thrive PlanNow that you’ve gotten all of your
packages
and
memberships
set up, you may be interested in learning more about which passes have been selling well, which are on the brink of expiry, which types of services are redeemed most often or even just a list of which patients have passes.
...
Settings
> **Treatments, Classes & Group Appointments).
Clinics will also be able to narrow down this report using the staff member, pass type, and date range filters.Subscribe to our monthly newsletter.Jane Software Inc.
...
- ![More from Jane](https://jane.app/assets/icons/stack_2_layers_dark_grey-7320907fa4e9bc366bdbf1c0f0851e53cd0eae755c37f0e143098652126300a5.svg)
- ![Feature Overview](https://jane.app/assets/icons/arrow_right_aqua-44fd64c285b864d54d7b80a7e51fe92c373aa86ded9e6e4fa710b8ba4995339f.svg)
- ![](https://jane.app/thumbs/467885ea-cda4-44df-bb6a-d6df95842228)
- ![](https://jane.app/thumbs/2d9e7910-f6eb-4d50-a93f-2c30d7b96c65)
