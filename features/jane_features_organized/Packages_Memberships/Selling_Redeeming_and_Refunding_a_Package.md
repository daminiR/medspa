...
*Source: [https://jane.app/guide/selling-redeeming-and-refunding-a-package](https://jane.app/guide/selling-redeeming-and-refunding-a-package)*

*Category: Packages & Memberships*

*Scraped: 2025-08-07 04:46:57*
...
Feature OverviewSelling, Redeeming and Refunding a Package
...
Note:
We always recommend checking with your regulating body to ensure this feature allows you to follow your local guidelines.
Read on, or feel free to jump straight to a particular section:
Selling a Package to a Patient
Redeeming a Package to a Patient
Removing a Package from an Appointment
No Shows and Packages
Reviewing a Patient’s Package
Package Expiry
Refunding a Package
Selling a Package to a Patient
...
Method #1 - Patient Billing
Head to
Patient Profile > Billing > Purchases
> Click
New Purchase
Method #2 - Patient Packages & Memberships
Head to
Patient Profile > Billing > Packages & Memberships
> Click
New Purchase
Method #3 - Appointment Panel
Head to
Schedule > Appointment Panel > Drop-Down
and select
Add Items
Regardless of how you access the Sale Page, you’ll be able to search for a package from the drop-down.
If your clinic compensates practitioners when the package is redeemed towards a session,
you will want to leave this purchase unassigned
— this is the more common scenario!
However, you will have the option of
assigning a staff member
to this sale if you wish for them to receive compensation for
the initial upfront purchase
of the package instead. If this is the case, to prevent compensating a provider twice for a particular package sale, you will want to be sure to unassign any upcoming appointments that will be covered by this package.
📍
NOTE
: Typically with many package arrangements, staff members are
not
compensated up front for the collected lump sum, but rather receive commission on the day that the package is redeemed towards an appointment. If this is the case for your clinic, leave this set to
No Staff Member.
You can receive a payment right away from this screen at the top of the page following the standard payment checkout workflow using the
Receive Payment
button at the top of the page.
Redeeming a Package Use for a Session
Now that your client has purchased their package, it is now available to be redeemed for any upcoming sessions. This means that the package can now be used to pay or cover the balance for the patient’s visit.
When booking a new appointment for the patient, if the treatment type selected falls under the
eligible items
for the package they have purchased, a new
Packages
section will appear. From here you can choose whether or not you would like to redeem a use of the package to cover this session or proceed as normal without using the package.
...
If a package is not applied during the time of booking, you will still be able to add it onto the appointment under the
Billing Info
section. This is also the same area where the package can be removed if it was applied by mistake.
Removing a Package from an Appointment
If you need to remove a package from a patient’s visit, you can head over to the appointment in the
Schedule
and scroll down to the
Billing Info
section of the
Appointment Panel
. Next, you’ll want to click on the blue
X
to the right of the package.
Another option would be to head over to the
Patient’s Profile
and click on
Appointments
. Once you find and select the appointment, you can scroll down to the
Billing Info
section and hit the blue
X
to the right of the package.
Note📝:
A package can only be removed from an appointment if it has not yet been arrived. These steps won’t delete the package entirely, just remove it for that one visit.
Once a package is applied and the appointment is
arrived
, the total billed to the patient will match the value that has been set under
Additional Charge
for this treatment or class type under the Package Settings.
Most often, clinics do not want to charge the patient anything additional per appointment, since the cost of the session had been covered by the upfront fee. In this case, the
Additional Charge
field would have been set to $0.00, and no further action is required since the appointment is now considered paid.
However, if a patient is to be charged a non-zero amount per appointment
in addition to
any upfront fee that was collected, the patient total will be adjusted to reflect this amount. In these cases, one would have to proceed with collecting any additional payment from the patient using the standard payment checkout procedure.
No Shows and Packages
...
Option 1:
Cover the no show by redeeming a package use for this session, or
Option 2:
Opt not to redeem a package use for this session, and choose one of the clinic’s existing
No Show Fees
to invoice for this appointment instead
In the case of
Option 1
, redeeming a package usage for a No Show would behave exactly the same way as if the appointment was arrived — the amount that the patient is billed will match whatever the
Additional Charge
amount has been set to in the Package settings. This is will also increment the total number of uses redeemed by one.
With
Option 2
, the package will not be used and instead, Jane will charge one of the standard clinic no show fees instead.
...
Reviewing
a Patient’s Packages
If you want to review the status of a patient’s package, you’ll be able to do so by visiting the
Patient’s Profile > Billing > Packages & Memberships
area. You will be able to review the date the package was purchased, how many uses are remaining, and when it will expire.
If you click on the
View
button, a detailed breakdown of the package will be available to review, including the specific invoices that the package has been redeemed towards.
There are also two reports, the
Package & Membership Sales and Package & Membership Usage Reports
that come in handy if you are interested in this type of information across all packages that have been sold and redeemed across the entire clinic.
Package Expiry
If a package has
expired
, it can no longer be redeemed towards an appointment, even if there are available sessions remaining.
From this show screen, clinics will be able to
modify
the expiration date of a package if desired. This can be used to either extend the validity of a package longer than the default preference if a patient requires an extension.
Leave the date field
blank
if you wish to set the package to no expiry.
If an appointment that is covered via a package is
rescheduled
to a date that is after the package expiry date, you will receive a notification indicating that the package must first be removed OR the expiry date must be adjusted prior to moving it.
Refunding a Package
You might run into a situation where a patient does not redeem all of the available uses of a package, and you would like to offer a refund of the remaining sessions. You would be able to issue them a partial package refund.
Refunding a package is a 2-step process, you’ll need to:
Initiate the refund - this leaves the funds as a credit on the patient’s account
Refund the payment - process the funds back to the patient’s credit card or mark the original payment as refunded
Step #1 - Initiating the Refund
❗️ Before initiating the refund,
ensure that the existing package is not attached to future appointments that have not happened yet.
For example, if you are refunding a package on a Monday and the patient has an upcoming appointment on Friday with the package attached, then that visit will count as one use and will be deducted from the amount you would be able to refund. You can remove the package by locating the appointment panel, scrolling to
Billing Info
and clicking
“X”
next to the package name.
Now that that is done, let’s get to initiating the refund! There are two ways you can initiate a package refund, and you have the choice to offer a partial refund in the process.
Method #1 - Purchase Page
Hop into the
Patient Profile
>
Billing
...
, click on the
drop-down arrow
next to the invoice and select Refund Package.
You will be presented with a pop-up asking to confirm whether you wish to refund this package, click
Initiate Refund.
This will leave the funds as a credit on their account, head to Step #2 to complete the refund.
Method #2 - Package Page
Head to
Patient Profile
>
Billing
...
There is a wide variation in refund regulations for package deals across different locales and disciplines. For this reason, the amount that you would like to return to the patient or client is flexible.
Jane provides a suggested partial refund amount, which is calculated by taking the original package price and dividing it by the number of uses to get a per-use value.
Jane then multiplies this amount by the number of remaining sessions to offer a suggested partial refund amount. However, you can override this value to any amount between $0 and the total amount originally collected.
Step #2 - Refunding the Package Payment
We’re not quite finished yet! The initial step in the process unapplies the payment from the package invoice and leaves it as a credit. For this next step in the process, we’ll need to refund the money back to the original payment method.
This can be done by viewing the original Payment itself — check out our guide on
Refunding a Patient Payment
for a quick refresher on how to finalize this second half of the refund.
📍
Note:
A friendly reminder that refunding the package will automatically
invalidate
the package, meaning that any remaining uses of the package can no longer be redeemed.
Video Walkthrough of Refunding a Package
If you have any questions about working with packages, don’t hesitate to send us an email to
support@jane.app
and we would be happy to guide you in the right direction.
...
Security
How to reset your email password and enable Two-Factor Authentication (2FA)Selling, Redeeming and Refunding a Package
Packages & Memberships
Selling, Redeeming and Refunding a Package
14 min read
Thrive Plan
...
If you have any questions about working with packages, don’t hesitate to send us an email to
support@jane.app
and we would be happy to guide you in the right direction.Subscribe to our monthly newsletter.Jane Software Inc.
...
- ![More from Jane](https://jane.app/assets/icons/stack_2_layers_dark_grey-7320907fa4e9bc366bdbf1c0f0851e53cd0eae755c37f0e143098652126300a5.svg)
- ![Feature Overview](https://jane.app/assets/icons/arrow_right_aqua-44fd64c285b864d54d7b80a7e51fe92c373aa86ded9e6e4fa710b8ba4995339f.svg)
- ![](https://jane.app/thumbs/b8196843-6fb1-4c48-87e6-558654761859)
- ![](https://jane.app/thumbs/1c332a88-4405-471c-90c3-9cb6959b246f)
- ![](https://jane.app/thumbs/d2ab9bae-62be-4599-8600-0a7e480818b3)
- ![](https://jane.app/thumbs/603f57c5-2c07-4b33-a5fb-8b1ffb1d445d)
- ![](https://jane.app/thumbs/1164d1d9-028c-4fe3-adb7-0df2ceb1151d)
- ![](https://jane.app/thumbs/9f332ac9-3f4d-4a0a-86d8-30d9c40bb3df)
- ![](https://jane.app/thumbs/30d8b1bb-80e8-44d3-85a2-c7cdad033c36)
- ![An image of the navigational steps and where to view the option to remove a package from a visit](https://jane.app/thumbs/d8032f6a-f1fb-42c2-8f6f-041c0b66685c)
- ![An image of the navigational steps and where to find the option to remove the package from the appointment](https://jane.app/thumbs/4d11ac28-24e5-4744-ad43-895294d18f3c)
- ![](https://jane.app/thumbs/079d668e-d300-4812-b14e-26dc2c51a04c)
- ![](https://jane.app/thumbs/89fa6c6c-91c9-4ed6-9f0c-70105dcbaf82)
- ![](https://jane.app/thumbs/0ba3f014-e25d-423c-816a-fc17c5baf3ae)
- ![](https://jane.app/thumbs/de900351-6a98-482d-8821-b79bf4074006)
- ![](https://jane.app/thumbs/792a29e8-5a7b-42e3-8d3d-1be8b3c08949)
- ![](https://jane.app/thumbs/84d7a9aa-687d-4b76-82fa-8d0906e3bb4e)
- ![](https://jane.app/thumbs/472752cc-7a75-4159-ab48-f71fffbf41f3)
- ![](https://jane.app/thumbs/e94eea94-b27e-4a6f-9603-5fe6dea9c3cc)
- ![](https://jane.app/thumbs/72a53ec4-30ab-45a2-b9ba-8cf63d218907)
- ![](https://jane.app/thumbs/472752cc-7a75-4159-ab48-f71fffbf41f3)
- ![](https://jane.app/thumbs/518ac183-babf-4e54-912c-b8b201c13d45)
- ![](https://jane.app/thumbs/5c0d405e-9620-4c55-a24a-865d965e895b)
