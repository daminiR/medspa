...
*Source: [https://jane.app/guide/importing-from-pmp](https://jane.app/guide/importing-from-pmp)*

*Category: Importing from Other Software*

*Scraped: 2025-08-06 23:49:36*
...
Feature OverviewImporting from PMP
...
Chiro
practice. Whether you’re thinking about making the switch or have already signed up for your new Jane account, our dedicated support teams are here to ensure your success in getting started with us.
It doesn’t stop there! After you’re settled in, you’ll always have
free
, ongoing support from the Jane team whenever you need it.
...
.
What can you expect?
Coming from PMP, we’ve previously brought over
patients
and
comments/notes
...
If you’ve previously migrated from PMP to OCA Aspire, please reference our guide on
importing from OCA Aspire
.
Exporting your Patient List from PMP
Coming from PMP, we’ll walk you through how to export your
complete list of patients
with as much detail as possible. To do so, please follow these steps:
Navigate to
Utilities
in the top menu >
Patient Export by Office
In the pop-up, click
Choose Criteria using Query
On the left-hand side, toggle ON both the
Age
checkboxes
Set the
Age >=
...
Accept
on the right
Back in the pop-up, click
Select Fields to Export
Click the
Copy Defaults
button on the left. Check to make sure all checkboxes in the
Available Fields
column on the right are checked off
Click the
Accept
button
Back in the pop-up, click
Export, Excel Spreadsheet
A message will pop up saying “Please Wait, collecting the Data”
Once the report runs, title the file
Patient List 0-20
and click
Save
...
with the remaining
age ranges
to export your complete list of patients from PMP:
21-40
,
41-60
,
...
💡
Helpful hint:
The above steps will export
ALL
patients from your PMP account. If you’re only looking to bring over patients for a specific doctor, you can select that individual from the
...
comments
section of patient profiles to enter notes for patients, these can be exported to Excel by following these steps:
Navigate to
Reports
>
Patient Profile
In the pop-up, click
Run Report by Clinic
Click the
Fill the List using Patient Query
button
Toggle ON the two
Age
checkboxes
...
Click
Run the Report
Once the report runs, save and title the file to include the
age range
(e.g. 0-20) in the filename
For
Save as type:
select
...
to clear the list, then populate it again by using
Fill the List using Patient Query
to set the remaining age ranges (21-40, 41-60, 61-80, 81-100+).
💡
Helpful hint:
xls
files (the older version of Excel files) are known to cut off at around 60,000+ rows. To double-check this, you can cross-reference the
page number
at the bottom of the Excel file with the number of pages listed in the file preview at step 9 to make sure the full number of pages is being included.
Appointments from PMP
Coming from PMP, the
Appointment Log
report exports to an unusable file format (.ace file) that we’re unable to work with. However, we may be able to work with the
RTF
version of this report. To export the Appointment Log to RTF, you can try these steps:
Navigate to the
Appointment Log
report
Select the
...
and
Date To
date to capture past and upcoming appointments
Select the
Export to RTF
option
Click
...
RTF file
to your device
If you receive an error message, you may need to filter for a smaller date range.
As a last resort, the
Patient History
report (Patient History > List of Patients)
may
be able to be used to bring over past appointment records, although this report contains a patient’s full billing history, so additional correspondence is required to import from these files and is not recommended.
Exporting anything else from PMP
For any other data, we recommend
reaching out directly to PMP
to inquire whether they are able to export additional data on your behalf.
Uploading into Jane
To ensure that your data is uploaded to the correct account, and for security reasons, we do not accept patient data via email.  You can upload the files to us in the
Settings
>
Schedule an Import
area.
...
If you need any further help getting set up to run with Jane,
just let us know
!
PMP Billing Data
We don’t import billing information as there are formatting issues between the two systems, but we can show you how to manually
...
Security
How to reset your email password and enable Two-Factor Authentication (2FA)Importing from PMP
Importing from Other Software
Importing from PMP
7 min read
Balance Plan
...
- ![](https://jane.app/thumbs/10ebf35a-bf57-40b4-b52e-c18ea24fb15f)
