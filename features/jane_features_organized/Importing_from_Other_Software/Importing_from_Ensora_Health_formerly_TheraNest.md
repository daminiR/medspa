...
*Source: [https://jane.app/guide/importing-from-theranest](https://jane.app/guide/importing-from-theranest)*

*Category: Importing from Other Software*

*Scraped: 2025-08-06 23:54:39*
...
Feature OverviewImporting from Ensora Health (formerly TheraNest)
...
behavioral health
practice. We have dedicated staff to ensure switching over is easy and to answer your questions every step of the way. We’ll help you set up your account so you can hit the ground running and guide you through importing your data.
It doesn’t stop there! After you’re settled in, you’ll always have free, ongoing support from the Jane team whenever you need it.
...
.
Moving from Ensora Health (formerly TheraNest) to Jane
While moving your practice to Jane, we’ve got your back every step of the way. Whether you’re thinking about making the move, or have already signed up for your new Jane account, our dedicated support teams are here to ensure your success in getting started with us.
...
What can you expect?
Coming from Ensora Health (formerly Theranest), we’ve previously brought over
clients
,
...
Session Notes & Documents
⚠️
An important note on moving from Ensora Health:
To ensure we’re able to import your clients, appointments, and session notes with accuracy, you’ll need to click the
Generate Client ID Number
button in
each
client’s profile
prior
to completing the steps below. The Client ID Number allows us to map the data with 100% accuracy.
Exporting your Client List from Ensora Health (formerly TheraNest)
Ensora Health allows you to export your
complete list
of clients fairly easily. To export the client list, you can follow the steps below:
Click on
Organization
...
Export Data
at the bottom
In the Export Data page, click the
Export all clients
button
The file will download to your device
The exported file will contain your complete list of clients, which includes both
active
and
archived/inactive
clients. If you only wish to bring over your
active clients
, you can click the
Export active clients
option instead.
Exporting your Appointments from Ensora Health
To export your complete list of past & upcoming appointments from Ensora Health, please follow these steps:
Click
Reports
in the left-hand menu
Navigate to the
Detailed Attendance (Beta)
report under the Appointments section
Using the
Date Range
filter, filter back to before you started using Ensora Health and then past today’s date into the
future
to capture all past and upcoming appointments
Click the
Run Report
button
...
(not PDF)
The file will download to your device
This report will capture your
complete set
of appointments for your clinic. In case you’re only looking to bring over a smaller set of appointments (for example, for a specific staff member), you can
filter down
your results by selecting single or multiple items from each column.
You can reference this guide from TheraNest on
How to Export this Data
for some extra support.
Exporting your Session Notes & Uploaded Documents from Ensora Health
To the best of our knowledge, Ensora Health does
not
offer the option to
bulk export
your session notes or uploaded documents & images. The first step will be to
reach out to Ensora Health
to request if there is the option for them to complete a bulk export for you. A reminder, it is your right as a clinic owner to have efficient access to your clients’ charting and files.
At this time, to bring over session notes and documents, we have seen clinics
individually download
these files from client profiles, which we are able to bring into Jane. To assist with the transition, these files can be provided at a later date after you’ve gotten up and running with Jane (see our recommended Timeline below).
To export the
session notes
for a client, please follow these steps:
Navigate to a client’s
profile
In the profile, click on their
Reports
tab
Select
Notes History
Click the
Print Notes History for All Cases
button
In the PDF preview,
copy
the
...
on your device and
paste
the client’s name and ID number as the folder name
Back in the PDF preview, click the
Download icon
Locate the folder you created for that client
Click
Save
Next, follow these steps to export their
documents
:
In the client’s
profile
...
tab
Click on a document
In the preview, click the
Download icon
Locate and save the file to
folder
you created for that client
Repeat
steps 2-4 for each document
You can read more about
How to Export This Data
from their help guide on this topic.
Naming your Files
...
FirstName_LastName_Identifier
You can choose which identifier makes sense for you and is easiest. This can be a unique patient number for the patient that’s present on the patient list export as well or a date of birth formatted
yyyymmdd
.
Should you want a bit more detail about that, scroll down to the
Charts
section of our
Basics of Importing to Jane
guide.
Ensora Health Billing Data
We don’t import historical billing information as there are formatting issues between the two systems, however, we have a great guide on
...
You can upload the
.zip file
(or files if there are multiple clinicians) as-is into the import along with any report files (Client Details, Appointment Status, etc.) directly into the import. If you password-protected your zip file, please remember to include the password in a
message
...
Security
How to reset your email password and enable Two-Factor Authentication (2FA)Importing from Ensora Health (formerly TheraNest)
Importing from Other Software
Importing from Ensora Health (formerly TheraNest)
8 min read
Balance Plan
...
💡Learn more about our
Balance Plan
, designed to be flexible for cash-based solo practitioners who book up to 20 appointments a month.Subscribe to our monthly newsletter.Jane Software Inc.
...
- ![More from Jane](https://jane.app/assets/icons/stack_2_layers_dark_grey-7320907fa4e9bc366bdbf1c0f0851e53cd0eae755c37f0e143098652126300a5.svg)
- ![Feature Overview](https://jane.app/assets/icons/arrow_right_aqua-44fd64c285b864d54d7b80a7e51fe92c373aa86ded9e6e4fa710b8ba4995339f.svg)
- ![](https://jane.app/system/dragonfly/production/2017/03/22/2jxcqzr4l5_blob)
