...
*Source: [https://jane.app/guide/importing-from-chirotouch-cloud-version](https://jane.app/guide/importing-from-chirotouch-cloud-version)*

*Category: Importing from Other Software*

*Scraped: 2025-08-06 23:39:46*
...
Feature OverviewImporting from ChiroTouch (Cloud Version)
...
What can you expect?
While you’re making the switch over to Jane, ChiroTouch Cloud allows you to export most of your data from their system. Below you’ll find step-by-step instructions for how to export your data when you’re feeling ready to make the switch to charting, billing and booking in Jane 🎉
What can we import?
Patient List
Appointments
Encounters & Uploaded Documents
Patient Alerts & Notes
Note:
We have not seen an
Insurance Policies
report from the Cloud version of ChiroTouch. We would recommend reaching out to their
support team
directly to inquire if there is a way to export this information.
Exporting your Patient List from
ChiroTouch Cloud
To export your patient demographics from ChiroTouch Cloud, you can follow the steps below:
From the
Analytics
dashboard, navigate to the
Patients
tab
...
or
Excel
In the panel that pops up, update the
Number of Records
value to ensure all patients are exported
Click
Export
Exporting your Appointments from
ChiroTouch Cloud
To export your appointment history from ChiroTouch Cloud, you can follow the steps below:
From the
Analytics
dashboard, navigate to the
Appointments
...
Update the
Number of Records
to a larger value to capture all appointments
Click
Export
💡
Helpful Hint:
It’s a good idea to double-check that the exported file contains the appointments for the filtered date range. If the full range of appointments is not being included, you can try breaking down the export into smaller date ranges (e.g. by year, 6 months, etc.)
Exporting your Patient Encounters & Documents/Files from
ChiroTouch Cloud
To export the
Patient Encounters
&
Documents/Files
for your clients, you can follow the steps below:
From the
Patients
tab, navigate to a
Patient’s Profile
...
PDF
Repeat
for any Documents/Uploaded Files from the
Documents
tab
🗂️ Naming your Files
To import the Encounters and downloaded Documents/Files, you will need to create a
folder
for each patient titled
FirstName_LastName_ChiroTouchID#
(e.g. Jane_Smith_CT10002) and drag their
Encounter PDF and any other files
into the folder. This allows us to accurately map the files to the correct patient. The ChiroTouch ID# is present in the
Patient Roster
file but can be replaced with another identifier such as
Birthdate
formatted
YYYY-MM-DD
if need be.
To specify the
chart author
for the files, you can create a separate folder for
each staff member
and then place all of their respective patients’ folders into that folder.
The final step will be to
zip
the folder for each staff member and upload the zip files into the import. We have a handy guide that helps walk you through
How to Make a Zip File
📁
Please Note:
We are unsure if there is an option to have ChiroTouch perform a
bulk Chart Export
of all
chart data
(Encounters + Documents) in the account. We would recommend reaching out directly to
ChiroTouch’s Support team
to inquire about this.
💡A note on exporting Insurance Policies from ChiroTouch Cloud
Coming from the Cloud version of ChiroTouch, we have heard from clinics that no report can be exported containing enough insurance policy information. To import policies to Jane, we would need a report that contains
Patient name
,
Insurer Name (Payor)
, and
Member ID number
. As far as we know, there isn’t a report in the Cloud version of ChiroTouch that contains this required information. We would recommend reaching out to their support team to inquire about this.
To bring over insurance policies, we have seen clinics add
two additional columns
to the Patient List file (Payor Name & Member ID#) and add this information in for each patient. Columns for secondary coverage can be added as well if needed.
Exporting your Patient Alerts & Notes from
ChiroTouch Cloud
To bring over patient alerts and notes, please follow these steps:
From the
Analytics
dashboard, navigate to the
Patients
...
In the panel that pops up, update the
Number of Records
value to ensure the full amount of notes are exported
Click
Export
💡
Helpful Hint:
If there are notes you would like to
exclude
, you can filter the exported file to remove them. Or, if there is any easy way for us to identify the notes you don’t want in Jane, you can specify that to us in a
message
in the import area when uploading your data.
Uploading into Jane
For security reasons, we do not accept patient data via email. You can upload the files to us in the secure import area in your Jane account under
Settings
>
Schedule an Import
>
...
(or
files
if there are multiple providers) along with the
report files
(Patient List, Appointment by Status, etc.) directly into the import.
...
Security
How to reset your email password and enable Two-Factor Authentication (2FA)Importing from ChiroTouch (Cloud Version)
Importing from Other Software
Importing from ChiroTouch (Cloud Version)
8 min read
Balance Plan
...
- ![](https://jane.app/thumbs/ffa81919-b4d0-4f5c-a8da-90d66c72ee81)
