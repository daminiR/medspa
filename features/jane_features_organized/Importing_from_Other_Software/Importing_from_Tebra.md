...
*Source: [https://jane.app/guide/importing-from-tebra](https://jane.app/guide/importing-from-tebra)*

*Category: Importing from Other Software*

*Scraped: 2025-08-06 23:54:13*
...
Feature OverviewImporting from Tebra
...
What can you expect?
Coming from the Tebra, we’ve successfully brought over
...
What can we import?
Patient List (with demographic data)
Payors & Insurance Policies
Chart Data + Files
Exporting your Patients from Tebra
To export your
Patient List
also known as the
Patient Demographic Export
from Tebra, please follow these steps:
Click
Reports > Patients > Patient Demographic Export
. The Patient Demographic Export window opens.
Click
Customize
. The Customize Report - Patient Demographic Export window opens.
Click the
Balance
...
Click
OK
. The Patient Demographic Export window returns and the report generates then displays Export Completed when finished.
Click
Excel or CSV
. The Save As dialog box opens then save the file as necessary in a secure folder.
If the
Active Vs Inactive
status is not provided in the patient list report, all clients are imported as
active
unless otherwise specified. This report also contains the data for
Payors
and
Insurance Policies
from what we have seen in the past - We can also import this data as part of the process ☀️
💡 Helpful hint: Tebra also has a guide on exporting this report in case that helps:
Patient Demographic Export
Exporting Appointments from Tebra
We have
not
seen this data come over from Tebra in the past. We would recommend reaching out to the
Support Team over at Tebra
to inquire if it is possible to export the data related to all past and future appointments for your practice.
Exporting your Payors & Insurance Policies from Tebra
Payor and insurance policy information can be imported into Jane. This data from what we have seen in the past is part of the
Patient Demographic Export
as mentioned above.
Exporting your Chart Notes from Tebra
From what we have seen in the past ➡️ Tebra can export patient clinical data in bulk. The patient clinical data exports as
individual XML Summary of Care files for patients with clinical notes created, signed, co-signed, or assigned by/to the selected provider during the timeframe specified
. In addition to the Summary of Care file, the export also includes patients’ documents.
‼️ We will need to review the XML data provided once uploaded to ensure it is
structured & mappable
with the rest of the data provided for this process.
To begin this process, please follow the steps below:
Hover over the
User icon
and click on
Practice Settings
. The Practice Settings page opens.
Click
Data Management
. Additional setting options expands.
Click
Export Patient Data
. The Export Patient Summaries page opens displaying one-time exports and recurring exports from the last 60 days. For this process, you would be using the
one-time export
option.
Click
Run Export
. The Run Export pop-up window opens.
Date range: Select a date range to include patients based on when they were seen by the provider.
Provider: Select
all providers
or a
specific provider
from the drop-down to include in the export.
Export Date: Select when to run the export.
Now: Click to select “Now” to generate the export now.
Date Range: Click to select the date range option. Then, select the date and time to generate the export.
Click Run Export. The requested export displays under the Patient Summaries list with the status of Pending.
Once the export is ready, the status changes to
Ready
and the user who initiated the export receives a notification via the Message Center.
Click the
download icon
to
save the zip file
which you would upload directly to Jane’s import area as part of the process.
💡 Helpful hint: If you happen to open the
zip file
before uploading it to Jane, please
re-zip
the file again before uploading it into the Import Request. You can read all about
Zipping Files Here
😊
...
author information
as is, and therefore when we import this data into Jane, the author of all chart entries defaults to a generically created
Admin Account
as part of the process. This can of course be merged into a staff profile you have created which would amalgamate all the data as needed. When completing the
Chart Export
from Tebra, you do have the option to export data
per practitioner
which may allow for further filtering as needed. Potentially, you could run the bulk export practitioner-by-practitioner and then
identify the corresponding zip file for each staff member
when uploading to Jane which would help us import the data specific to each practitioner as the
author
of the chart data.
...
>
View Details
to securely upload the data you exported from Tebra.
...
Tebra Billing Data
We aren’t able to import billing information such as Accounts Receivable or Credits as there are formatting issues between the two systems, but our understanding is that Tebra allows for the export of your billing data to back up files prior to your subscription expiring, so you can retain access to this information after your Tebra access has ended. We would recommend reaching out to the
Support Team over at Tebra
for further support with this.
...
Security
How to reset your email password and enable Two-Factor Authentication (2FA)Importing from Tebra
Importing from Other Software
Importing from Tebra
9 min read
Balance Plan
